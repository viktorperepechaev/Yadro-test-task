...
**Этап 1** — максимизировать количество обслуженных потребителей:
$$\max \sum_{c \in C} v_{c,t} \quad \text{при всех ограничениях выше}$$

Обозначим оптимальное значение $V^*$.

**Этап 2** — минимизировать затраты на генерацию, зафиксировав $V^*$:
$$\min \sum_{g \in G} \text{cost}_g \cdot p_{g,t}$$
$$\text{при всех ограничениях выше, плюс} \quad \sum_{c \in C} v_{c,t} \geq V^*$$
