...
**Версии библиотек** (фиксированы в `uv.lock`):

| Библиотека   | Версия | Назначение               |
|--------------|--------|--------------------------|
| `pulp`       | 3.3.1  | MILP-солвер (CBC)        |
| `matplotlib` | 3.10.9 | визуализация             |
| `pytest`     | 9.0.3  | тесты (dev-зависимость)  |
...
- **Визуальная проверка и ручная доработка визуализации:** сгенерированные PNG просматривались вручную. По итогам просмотра тепловая карта была доработана вручную: бинарная раскраска (зелёный/красный) заменена на градиентную — яркость ячейки кодирует фактический спрос потребителя в данный час, нормировка переведена с побочной (относительно каждого потребителя) на глобальную, чтобы потребители были сравнимы между собой. Также была изменена сортировка строк: по пиковому часовому спросу вместо суммарного суточного.
